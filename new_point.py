...
from PyQt5.QtCore import Qt, QEvent, QByteArray, QSize, QRectF, QTimer
...
from dialogs import ConstructionConfigDialog, CurveDialog, ZeroLineDialog, MaterialLineDialog
...
# ===========================================================================================================================
    def initUI(self):
        ...
        left_section.setStyleSheet("QFrame { border: 2px solid #4CAF50; border-radius: 10px; background-color: #E8F5E9; margin: 10px; }")
        left_section.setMinimumWidth(380)
        left_section.setMaximumWidth(420)
        left_layout = QVBoxLayout(left_section)
        left_layout.setContentsMargins(10, 5, 10, 5)
        left_title = QLabel("Settings Section")
        left_title.setAlignment(Qt.AlignCenter)
        left_title.setStyleSheet("font-weight: bold; font-size: 14px; padding: 10px;")
        left_layout.addWidget(left_title)

        # ------------------------------ NEW WORKSHEET BUTTON --------------------------------
        self.new_worksheet_button = QPushButton("New Worksheet")
        self.new_worksheet_button.setStyleSheet("""
            QPushButton {
        ...
        """)
        #self.new_worksheet_button.clicked.connect(self.new_worksheet)
        left_layout.addWidget(self.new_worksheet_button)

        # -------------------------------------------------------------------------------------
        # Add metrics selection dropdown at the top
        self.project_group = QGroupBox("Select Projects")
        project_layout = QHBoxLayout()
        self.project_combo = QComboBox()
        self.project_combo.addItems(["Project - 1", "Project - 2", "Project - 3"])
        self.project_combo.currentTextChanged.connect(self.update_measurement_metrics)
        ...
        # ----------------- 3D POINT CLOUD FILE LOAD BUTTON ----------------
        self.load_button = QPushButton("Load Point Cloud Data")
        ...
        operation_frame.setFrameStyle(QFrame.Box | QFrame.Raised)
        ...
        operation_title = QLabel("Operation Section")
        operation_title.setAlignment(Qt.AlignCenter)
        ...
        # --------------------- Design Configuration Button ---------------------
        self.design_button = QPushButton("Design Configuration")
        ...
        """)
        self.design_button.clicked.connect(self.toggle_design_buttons)
        operation_layout.addWidget(self.design_button)

        # Container for Road & Bridge Baseline buttons (initially hidden)
        self.design_subbuttons_widget = QWidget()
        self.design_subbuttons_layout = QVBoxLayout(self.design_subbuttons_widget)
        self.design_subbuttons_layout.setContentsMargins(20, 5, 20, 10)  # Indent slightly
        self.design_subbuttons_layout.setSpacing(8)

        # Road Baseline Button (Purple, Narrower)
        self.road_baseline = QPushButton("Road Baseline")
        self.road_baseline.setStyleSheet("""
            QPushButton {
            ...
            QPushButton:pressed { background-color: #8E24AA; }
        """)
        self.road_baseline.setVisible(False)
        self.road_baseline.clicked.connect(self.toggle_road_baseline_checkboxes)
        self.design_subbuttons_layout.addWidget(self.road_baseline, alignment=Qt.AlignCenter)

        # Bridge Baseline Button (Purple, Narrower)
        self.bridge_baseline = QPushButton("Bridge Baseline")
        self.bridge_baseline.setStyleSheet("""
            QPushButton {
                background-color: #AB47BC;
                color: white;
                border: none;
            ...
            QPushButton:pressed { background-color: #8E24AA; }
        """)
        self.bridge_baseline.setVisible(False)
        self.bridge_baseline.clicked.connect(self.toggle_bridge_baseline_checkboxes)
        self.design_subbuttons_layout.addWidget(self.bridge_baseline, alignment=Qt.AlignCenter)

        # Add the container (initially hidden)
        self.design_subbuttons_widget.setVisible(False)
        operation_layout.addWidget(self.design_subbuttons_widget)

        # --------------------- Construction Configuration ---------------------
        self.material_line = QPushButton("Construction Configuration")
        self.material_line.setStyleSheet("""
            ...
            QPushButton:pressed { background-color: #006f9a; }
        """)
        self.material_line.clicked.connect(self.toggle_material_line_checkboxes)
        operation_layout.addWidget(self.material_line)

        # --------------------- Measurement Button ---------------------
        self.measurement = QPushButton("Measurement")
        self.measurement.setStyleSheet("""
            ...
            QPushButton:pressed { background-color: #006f9a; }
        """)
        self.measurement.clicked.connect(self.toggle_measurement_buttons)
        operation_layout.addWidget(self.measurement)

        # --------------------- Measurement Metrics (Hidden Initially) ---------------------
        ...
        operation_layout.addWidget(self.metrics_group)

        # --------------------- Measurement Tools (Hidden Initially) ---------------------
        self.line_button = QPushButton("Line")
        self.line_button.setVisible(False)
        self.line_menu = QMenu()
        self.vertical_line_action = QAction("Vertical Line", self)
        self.horizontal_line_action = QAction("Horizontal Line", self)
        ...
        operation_layout.addWidget(self.complete_polygon_button)

        # Add stretch to push everything up
        operation_layout.addStretch()

        # Finally add the operation frame to left layout
        left_layout.addWidget(operation_frame)
  
        # ==================== MESSAGE SECTION (COLLAPSIBLE) ====================
        self.message_button = QPushButton("Message")
        self.message_button.setStyleSheet("""
            QPushButton { background-color: #FF8A65; color: white; border: none; padding: 8px;
        ...
        self.message_section.setStyleSheet("""
            QFrame { border: 2px solid #FF5722; border-radius: 8px; background-color: #FFF3E0; margin: 5px 10px; }
        """)
        ...
        self.message_text.setStyleSheet("""
            QTextEdit { background-color: #FFF8E1; border: 1px solid #FF8A65; border-radius: 5px;
        ...
        # Create a container for the reset buttons
        self.reset_buttons_container = QWidget()
        self.reset_buttons_layout = QHBoxLayout()
        self.reset_buttons_container.setLayout(self.reset_buttons_layout)
        ...
        """)
        top_section.setMaximumHeight(80)
        ...
        # ------------------------------------------------------------------
        # SCALE SECTION – New section between middle and bottom
        # ------------------------------------------------------------------
        ...
        self.volume_slider.setStyleSheet("""
            QSlider {
                padding-left: 16px;    /* Match left padding with scale graph */
                padding-right: 17px;   /* Match right padding with scale graph */
                margin: 2px 2px;
            }
            ...
            QSlider::handle:horizontal {
                ...
                width: 18px;
                height: 18px;
                margin: -6px 0;  /* (18px handle - 6px groove = 12px / 2 = 6px) */
                border-radius: 9px;
            }
        ...
        # In initUI method, when creating the scale section:
        self.scale_figure = Figure(dpi=100)
        self.scale_figure.set_size_inches(8, 1.2)  # Wider aspect ratio for scale
        self.scale_canvas = FigureCanvas(self.scale_figure)
        self.scale_canvas.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        self.scale_canvas.setMinimumHeight(120)  # Increased from 80
        self.scale_canvas.setMaximumHeight(140)  # Increased from 100

        # Initialize the scale axes
        self.scale_ax = self.scale_figure.add_subplot(111)
        self.scale_ax.set_xlim(0, self.total_distance)
        self.scale_ax.set_ylim(0, 1.2)  # Changed from 1.5 to 1.2 to provide space for label

        # Add this line after setting ylim:
        self.scale_ax.set_facecolor('#FFF3E0')
        self.scale_ax.set_xlabel('Chainage', labelpad=3)  # Added labelpad
        ...
        # Set initial ticks (will be updated when zero line is set)
        self.scale_ax.set_xticks([])
        self.scale_ax.set_xticklabels([])

        # Adjust tick parameters to prevent cutting
        ...
        self.scale_figure.tight_layout(rect=[0, 0.1, 1, 0.95])  # Adjusted rect parameters
        ...
        # Store reference to scale section
        ...
        # Line Section
        # ---------------------------------------------------------------------
        # Line Section: (empty now, checkboxes moved to left)
        line_section = QFrame()
...
# -------------------------------------------------------------------------------------------------------------
        # Create checkbox rows using the professional style with pencil icons
        def create_line_checkbox_with_pencil(checkbox_text, info_text, item_id, color_style=""):
            """Create a checkbox row with pencil icon"""
            container = QWidget()
            ...
            layout.setSpacing(5)
            # Create checkbox - SIMPLIFIED styling
            checkbox = QCheckBox()
            checkbox.setFixedSize(35, 35)
            # Remove complex stylesheet that might be hiding the checkbox
            checkbox.setText("") # Ensure no text on checkbox itself
            checkbox.setObjectName(item_id)
            # Create pencil button with SVG icon
            ...
            # Create text label with color styling
        ...
        # Apply color styling to the checkbox instead of label
        ...
        # Bridge Specific Lines
        # -------------------------------------------------------------------------------------------------------------
        # Bridge-specific Zero Line (different from road zero line)
        ...
        # Apply color styling to the checkbox
        ...
        self.bridge_zero_pencil.clicked.connect(self.edit_zero_line)  # You might want a separate function for bridge zero line
        ...
        self.preview_button.setStyleSheet("""
            ...
            QPushButton:hover { background-color: #6E6E6E; } /* Slightly darker */
            QPushButton:pressed { background-color: #5A5A5A; } /* More dark */
        """)
        ...
        # ------------------------------------- Graph Canvas --------------------------------------------
        # Matplotlib graph canvas (embedded above volume slider)
        self.figure = Figure(dpi=100)
        self.figure.set_size_inches(self.total_distance / 10.0, 6)
        self.canvas = FigureCanvas(self.figure)
        self.canvas.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        # Initialize the axes with grid, zero at center, x at 5m, y at 1m intervals
        ...
        # Set major ticks every 5 units (meters) for x-axis
        self.ax.xaxis.set_major_locator(ticker.MultipleLocator(5))
        # Set major ticks every 1 unit (meter) for y-axis
        self.ax.yaxis.set_major_locator(ticker.MultipleLocator(1))
        # Add annotation for hover coordinates
        ...
        # Get reference to the horizontal scrollbar
        ...
        content_layout.addWidget(line_section)
        content_layout.addWidget(self.graph_scroll_area, 1)
        bottom_layout.addLayout(content_layout)
        # ------------------------------------------------------------------
        # RIGHT SECTION – Stack Visualization and Bottom vertically (Right Section is the combination of Middle and Bottom)
        # ------------------------------------------------------------------
        right_section = QWidget()
        right_section.setSizePolicy(
            QSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        )
        ...
        right_layout.addWidget(middle_section, 3) # Give more space to visualization
        right_layout.addWidget(scale_section, 1)
        ...
        # At the end of initUI method, after all UI setup:
    ...
    # -------------------------------------------------
    # TOGGLE BASELINE CHECKBOXES
    def toggle_road_baseline_checkboxes(self):
        # Check if road baseline is currently active
        if self.road_baseline.text() == "Hide Road Baseline":
            # Road baseline is currently active, so hide road checkboxes
            self.road_surface_container.setVisible(False)
            self.surface_container.setVisible(False)
            self.zero_container.setVisible(False)
            self.construction_container.setVisible(False)
            ...
            self.threed_map_button.setVisible(False)
            self.save_button.setVisible(False)
            # Inside toggle_road_baseline_checkboxes() when showing road mode:
            ...
            if self.current_mode == 'road':
                self.save_current_lines_state()
            
            self.current_mode = None
        else:
            # Road baseline is not active
            # First hide any bridge checkboxes if they're active
            if self.bridge_baseline.text() == "Hide Bridge Baseline":
                self.save_current_lines_state()  # Save bridge lines
                self.clear_graph_for_switch()
            ...
            self.road_baseline.setText("Hide Road Baseline")
            self.preview_button.setVisible(True)
            self.threed_map_button.setVisible(True)
            self.save_button.setVisible(True)
            
            # Set current mode to road
            self.current_mode = 'road'
            
            # Update graph title
            self.ax.set_title('Road Construction Layers', fontsize=14, fontweight='bold', color='#4A148C')
            
            # Restore saved road lines if any
            self.restore_saved_lines_state('road')
            ...
            # Add message
            self.message_text.append("Road Baseline Activated")
            self.message_text.append("Graph canvas set for road construction layers")

    def toggle_bridge_baseline_checkboxes(self):
        # Check if bridge baseline is currently active
        if self.bridge_baseline.text() == "Hide Bridge Baseline":
            # Bridge baseline is currently active, so hide bridge checkboxes
            self.deck_line_container.setVisible(False)
            self.projection_container.setVisible(False)
            if hasattr(self, 'bridge_zero_container'):
                self.bridge_zero_container.setVisible(False)
            ...
            # Inside toggle_bridge_baseline_checkboxes() when showing bridge mode:
            self.bottom_section.setVisible(True)
            
            # Reset construction dots state
            ...
            # Bridge baseline is not active
            # First hide any road checkboxes if they're active
            if self.road_baseline.text() == "Hide Road Baseline":
                self.save_current_lines_state()  # Save road lines
                self.clear_graph_for_switch()
            ...
            # Now show BRIDGE checkboxes ONLY
            ...
            self.bridge_baseline.setText("Hide Bridge Baseline")
            self.preview_button.setVisible(True)
            self.threed_map_button.setVisible(True)
            self.save_button.setVisible(True)
            
            # Set current mode to bridge
            self.current_mode = 'bridge'
            
            # Update graph title
            self.ax.set_title('Bridge Construction Layers', fontsize=14, fontweight='bold', color='#1565C0')
            
            # Restore saved bridge lines if any
            self.restore_saved_lines_state('bridge')
            ...
            # Add message
            self.message_text.append("Bridge Baseline Activated")
            self.message_text.append("Graph canvas set for bridge construction layers")
            self.message_text.append("Construction Dots available for bridge construction points")

# ---------------------------------------------------------------------------------------------------------------            
    def toggle_design_buttons(self):
        visible = not self.design_subbuttons_widget.isVisible()
        self.design_subbuttons_widget.setVisible(visible)
        self.road_baseline.setVisible(visible)
        self.bridge_baseline.setVisible(visible)

        if visible:
            # Hide measurement tools when entering design mode
            self.metrics_group.setVisible(False)
            self.line_button.setVisible(False)
            self.presized_button.setVisible(False)
            self.polygon_button.setVisible(False)
            ...
            # SHOW BOTTOM SECTION
            self.bottom_section.setVisible(True)
            self.message_text.append("Design Configuration Mode - Graph Visible")
        else:
            # Optional: hide road/bridge checkboxes when collapsing
            self.road_baseline.setVisible(False)
            self.bridge_baseline.setVisible(False)

        self.design_button.setText("Design Configuration ▼" if visible else "Design Configuration")

# ---------------------------------------------------------------------------------------------------------------

    def toggle_measurement_buttons(self, hide=False):
        """Toggle measurement tools and HIDE bottom section when active"""
        if hide:
            state = False
        else:
            state = not self.metrics_group.isVisible()

        # Show/hide measurement widgets
        self.metrics_group.setVisible(state)
        self.line_button.setVisible(state)
        self.presized_button.setVisible(state)
        self.polygon_button.setVisible(state)
        self.stockpile_polygon_button.setVisible(state)
        self.complete_polygon_button.setVisible(state)

        # CRITICAL: Hide bottom section when Measurement is active
        if state:
            self.bottom_section.setVisible(False)
            # Optional: Hide design/material sections
            self.design_subbuttons_widget.setVisible(False)
            self.road_baseline.setVisible(False)
            self.bridge_baseline.setVisible(False)
            ...
            self.message_text.append("Measurement Mode Activated - Graph Hidden")
        else:
            # Only show bottom if we're not in design/material mode
            if self.current_mode in ['road', 'bridge', 'material']:
                self.bottom_section.setVisible(True)

        self.design_button.setText("Design Configuration")


# ---------------------------------------------------------------------------------------------------------------
# Update the toggle_material_line_checkboxes method:
    def toggle_material_line_checkboxes(self):
        """Toggle material line section and SHOW bottom section"""
        if self.material_line.text() == "Hide Material Line":
            # Hide material line section
            self.material_line.setText("Construction Configuration")
            self.hide_material_section()
            # Do NOT show bottom section here unless another mode is active
        else:
            # Show material line section
            self.material_line.setText("Hide Material Line")

            # Hide other baseline sections
            if self.road_baseline.text() == "Hide Road Baseline":
                self.toggle_road_baseline_checkboxes()
            if self.bridge_baseline.text() == "Hide Bridge Baseline":
                self.toggle_bridge_baseline_checkboxes()

            # Hide measurement tools
            self.metrics_group.setVisible(False)
            self.line_button.setVisible(False)
            self.presized_button.setVisible(False)
            self.polygon_button.setVisible(False)
            ...
            self.show_material_section()

            self.message_text.append("Construction Configuration Mode Activated - Graph Visible")
            
            # Add message
            self.message_text.append("Material Line Section Activated")
            self.message_text.append("Click 'Add Material Line' to configure new materials")
    ...
    def on_checkbox_changed(self, state, line_type):
        if state == Qt.Checked:
            if line_type == 'construction_dots':
                ...
                if self.bridge_baseline.text() != "Hide Bridge Baseline":
                    QMessageBox.warning(self, "Bridge Baseline Required", 
                                    "Please activate Bridge Baseline first to use Construction Dots.")
                    self.construction_dots_line.setChecked(False)
                    return
...
# Define function for the connect the points which is already drawn on point cloud data:
    def connect_signals(self):
        ...
        self.preview_button.clicked.connect(self.on_curve_button_clicked) # Connect Curve button
